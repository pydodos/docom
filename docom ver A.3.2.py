...
while z == 100:
    ...
    if(a == '/?' or a == '/help'):
        ...
        print('      만든 사람 : 김도훈\n     만든 사람의 말:명령어 아이디어 고갈')
    elif(a == '/ver'):
        print('      ver a3. 2, 최신버전입니다.')
    elif(a == '/rcp'):
        d = 100
        while d == 100:
            a = ['가위', '바위', '보']
        ...
        print("현재 시간 :", now.year, '년', now.month, '월', now.day, '일', now.hour, "시", now.minute, "분", now.second, "초")
    elif(a == '/updown'):
        a = random_.randint(1, 100)
        while True:
            b = int(input('··· '))
            if b > a:
                print('down')
            ...
            c += 1
            if c == 7:
                print('7번 안에 맞추지 못하였습니다. 정답 = %s' % a)
                break
            if b == a:
                print('축하합니다! 정답입니다. %s번 만에 맞추었습니다.' % c)
                break
    else:
